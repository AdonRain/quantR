...
if(date.start < date.end){
  date.period <- seq(date.start, date.end, 'day');
  http.url <- 'http://www.csindex.com.cn/sseportal/csiportal/syl/hytype.do?code=3&zb_flg=2&db_type=0';
  http.header <- c(
  ...
  urls <- lapply(date.period, function(day) {
    return(paste(http.url, paste('date', day, sep = '='), sep = '&'));
  });

  htmls <- getURL(urls, httpheader = http.header, .encoding="gbk");

  pel <- lapply(htmls, function(html) {
